...
DynamicSceneObject::DynamicSceneObject(const int maxNumVertex, const int maxNumIndex, const bool normalFlag,
                                       const bool uvFlag)
{
    ...
    int byteOffset = 0;
    glVertexAttribPointer(SceneManager::Instance()->m_vertexHandle, 3, GL_FLOAT, false, strideV * 4,
                          (void*)(byteOffset));
    byteOffset = byteOffset + 12;
    glEnableVertexAttribArray(SceneManager::Instance()->m_vertexHandle);
    if (normalFlag)
    {
        glVertexAttribPointer(SceneManager::Instance()->m_normalHandle, 3, GL_FLOAT, false, strideV * 4,
                              (void*)(byteOffset));
        byteOffset = byteOffset + 12;
    ...
    if (uvFlag)
    {
        glVertexAttribPointer(SceneManager::Instance()->m_uvHandle, 3, GL_FLOAT, false, strideV * 4,
                              (void*)(byteOffset));
        byteOffset = byteOffset + 12;
...
float* DynamicSceneObject::dataBuffer()
{
    return this->m_dataBuffer;
}

unsigned int* DynamicSceneObject::indexBuffer()
{
